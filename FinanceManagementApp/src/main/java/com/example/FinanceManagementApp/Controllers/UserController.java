...
public class UserController {
    ...
    public ResponseEntity<Map<String,String>> signUp(@RequestBody Users user){
        ...
        if(userObj.getUser_id() != 0){
            ...
            response.put("User id",String.valueOf(userObj.getUser_id()));
    ...
    public ResponseEntity<Map<String,String>> login(@RequestBody Users user){
        Map<String,String> userObj = userRepo.loginValidation(user.getEmail(), user.getPassword());

        Map<String,String> response = new HashMap<>();

        if(userObj.get(0). >0){
            response.put("Status","Sign in success");
            response.put("User id",String.valueOf(userObj.get(0).ge);
            response.put("Name",String.valueOf(userObj.getName()));
        }*/
    @PostMapping("/signIn")
